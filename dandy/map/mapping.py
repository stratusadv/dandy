...
class Mapping(BaseModel):
    ...
    _keyed_mapping: Dict[str, tuple[str, Any]] = PrivateAttr(default_factory=dict)
    ...
    def __getitem__(self, item):
        return self.valid_mapping[item]
    ...
    def process_valid_to_keyed(self):
        for i, (choice, value) in enumerate(self.valid_mapping.items(), start=1):
            key = str(i)
            if isinstance(value, dict):
                self._keyed_mapping[key] = (choice, self.process_valid_to_keyed())
            else:
