...
1. Importing `dandy` instantiates a `DandySettings` singleton (`dandy/conf.py`) that imports a user settings module named by `DANDY_SETTINGS_MODULE` env var, defaulting to `dandy_settings` (`dandy/core/utils.py`, `dandy/constants.py`).
...
6. **`DEBUG` mode**: if `DEBUG = True` in settings, `tests/dandy_settings.py` (and `dandy_settings.py` templates) do `from dandy.core.debug import *`, which replaces `warnings.showwarning` with a stack-trace-printing handler used to debug LLM retry/validation issues.
...
| `cli/` | TUI (`blessed`), `session`, pluggable `/actions` (code/explain/help/quit) |
| `tool/` | `BaseTool` (tool descriptors, base-level) + `BaseSubprocessTool`/`GitTool` (subprocess wrapper) |
| `conf.py`, `constants.py`, `default_settings.py` | Settings singleton, version/constants, defaults |

Convention: every subsystem keeps its supporting pydantic models, bots, and user-facing prompts under a nested **`intelligence/`** directory (e.g. `dandy/llm/decoder/intelligence/`, `dandy/cli/actions/help/intelligence/`, `tests/bot/intelligence/`). Follow this when adding fakes or fixtures.
...
- **`LlmOptions`** (`dandy/llm/options.py`, a pydantic model with `extra='allow'`): `frequency_penalty`, `presence_penalty`, `top_p`, `temperature`, `max_completion_tokens`, `prompt_retry_count` (default `2`). Non-None values are range-checked (e.g. temperature 0–2, `max_completion_tokens` 1–99,999,999); violations raise `LlmCriticalError`.
...
- **Execute-and-loop**: `bot.llm.tools.prompt_to_intel(prompt, intel_class, tools=[...], tool_functions={'name': handler}, max_tool_iterations=5)` (`LlmToolService`, `dandy/llm/tool/service.py`) keeps calling the model, executing any requested tools (an explicit `tool_functions` entry wins over the tool's own `handle()`), feeding results back as `role='tool'` messages, until the model returns the final Intel. The original prompt is only sent on the first iteration (history carries it after that).
...
- `Recorder` is a `Singleton` (`core/singleton.py`). `Recorder.start_recording(name)` … `stop_recording(name)`, then `to_html_file` / `to_json_file` / `to_markdown_file` (renderers under `recorder/renderer/`, HTML templates under `html_templates/`). Recordings write to `{BASE_PATH}/{DANDY_DIRECTORY}/recordings/`; requires `ALLOW_RECORDING_TO_FILE = True`.
...
- **The CLI code action is a real tool-calling consumer**: `CodingBot` (`dandy/cli/actions/code/intelligence/bots/coding_bot.py`) calls `self.llm.tools.prompt_to_intel(... tools=CODE_EDITING_TOOLS ...)`. The CLI tools (`dandy/cli/actions/code/tools/`) are `list_directory`, `read_file`, `write_file`, `edit_file`, `delete_file`, `create_directory`; they resolve every path against `session.project_base_path` and refuse paths that escape it. `handle()` never raises — it returns an error string so the model can correct its arguments. Covered hermetically in `tests/cli/actions/code/` (unit tests + a mocked `request_to_response` loop proving the bot really edits files).
