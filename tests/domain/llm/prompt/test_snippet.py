...
from dandy.domain.llm.prompt import snippet
...
class TestSnippet(TestCase):
    ...
    def test_file_snippet(self):
        file_snippet = snippet.FileSnippet(
            file_path=Path(Path(__file__).parent.parent.parent.parent.resolve(), 'assets/documents/complex_markdown.md')
        )
    ...
    def test_object_source_snippet(self):
        object_source = snippet.ObjectSourceSnippet(
            object_module_name='dandy.application.bot.bot.Bot'
        )
