...
from dandy.domain.intel.exceptions import IntelCriticalError
from tests.domain.intel.intelligence.intel import ThingIntel, BagIntel, PersonIntel
...
class TestBaseIntel(TestCase):
    ...
    def test_base_intel_include_deeper_json_schema(self):
        PersonCopy = PersonIntel.model_inc_ex_class_copy(
            include={
                "first_name": True,
                "last_name": True,
                "bag": {"color": True, "stylish": True, "things": {"name": True}},
            }
        )
        ...
        self.assertNotIn(
            "description", json_schema["$defs"]["ThingIntel"]["properties"]
        )
    ...
    def test_base_intel_exclude_deep_json_schema(self):
        PersonCopy = PersonIntel.model_inc_ex_class_copy(
            exclude={"middle_name": True, "bag": {"things": True}}
        )
    ...
    def test_base_intel_exclude_deeper_json_schema(self):
        PersonCopy = PersonIntel.model_inc_ex_class_copy(
            exclude={
                "middle_name": True,
                "bag": {"pockets": True, "things": {"description": True}},
            }
        )
